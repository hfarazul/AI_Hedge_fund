...
def portfolio_management_agent(state: AgentState):
    ...
    # Validate the values after receiving them
    quantity = max(0, int(result.quantity))  # Ensure non-negative
    confidence = max(0.0, min(1.0, float(result.confidence)))  # Clamp between 0 and 1
